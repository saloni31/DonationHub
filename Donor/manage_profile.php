...
if (isset($_POST['add'])) {
    ...
    if (isset($firstname) && isset($address) && isset($mobile) && isset($email) && isset($city) && isset($area)) {
        $sql1 = "update donor set fullName='$firstname',contactno='$mobile',address='$address',city='$city',area_name='$area' where donor_id=" . $_SESSION['user']['id'];
...
<script>
    $(document).ready(function () {
        ...
        var pos = $.post("../Volunteer/data.php", {city_id: city_id, vol_id: vol_id, area_id: area_id});
        $("#area").empty();
        pos.done(function (data) {
            $("#area").append(data);
        });
